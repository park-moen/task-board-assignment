...
2. 현재 캐시(`previousTasks`)를 스냅샷으로 저장한 뒤, `moveTask` 순수 함수로 캐시를 낙관적으로 갱신합니다.
3. 요청이 실패하면 `onError`에서 저장해둔 스냅샷을 그대로 복원하고, `useToast`로 실패를 알립니다.
...
- 되돌릴 정확한 "이전 값"을 이미 `onMutate` 시점에 확보해뒀으므로, 굳이 서버에 다시 요청하지 않아도 됩니다.
